...
emarket.wallets.curve25519 = function () {}

var the = emarket.wallets.curve25519;

the.clamp = function(curve) {
  curve[0] &= 0xFFF8;
  curve[15] &= 0x7FFF;
  curve[15] |= 0x4000;
  return curve;
...
the.convertToByteArray = function(a) {
...
the.convertToShortArray = function(a) {
	var b = new Array(16);
	var i, val1, val2;
	for (i=0; i<16; i++)
	{
...
the.fillShortArray = function(src, dest) {
	var i;
	for (i=0; i<16; i++)
	{
		dest[i] = src[i];
...
the.fillByteArray = function(src, dest) {
	var i;
	for (i=0; i<32; i++)
	{
		dest[i] = src[i];
...
the.create = function(f, c, s) {
  ...
  if (s != null)
  {
	  ...
	  var stmp = the.convertToShortArray(sb);
	  the.fillShortArray(stmp, s);
  }

  return q[0];
}

the.keygen = function(s, curve) {
	the.clamp(curve);
	return the.create(curve, curve25519_nine(), s);
}
...
the.verify = function(Y, v, h, P) {
	...
	var p = [ curve25519_nine(), the.convertToShortArray(P) ];
	...
	var YY = the.convertToByteArray(t1[1]);
	the.fillByteArray(YY, Y);
}
